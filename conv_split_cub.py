...
MODELS = ['VAN', 'PI', 'EWC', 'MAS', 'RWALK', 'A-GEM', 'MEGA', 'MEGAD'] #List of valid models
IMP_METHOD = 'PI'
SYNAP_STGTH = 75000
...
def train_task_sequence(model, sess, saver, datasets, cross_validate_mode, train_single_epoch, do_sampling, is_herding,  
    ...
    for runid in range(num_runs):
        ...
        if model.imp_method == 'A-GEM' or model.imp_method == 'MEGA' or model.imp_method == 'MEGAD':
        ...
        for task in range(len(task_labels)):
            ...
            if task < (len(task_labels) - 1):
                ...
                if 'GEM' in model.imp_method or 'MEGA' in model.imp_method or 'MEGAD' in model.imp_method: 
                    data_to_sample_from = {
...
def main():
    ...
    for imp_method in models_list:
        ...
        elif imp_method == 'A-GEM' or imp_method == 'MEGA' or imp_method == 'MEGAD':
            synap_stgth_list = [0]
            if args.online_cross_val or args.cross_validate_mode:
                pass
            else:
