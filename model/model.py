...
class Model:
    ...
    def __init__(self, x_train, y_, num_tasks, opt, imp_method, synap_stgth, fisher_update_after, fisher_ema_decay, network_arch='FC-S',
        ...
        if imp_method == 'PNN':
            ...
            if (imp_method == 'A-GEM' or imp_method == 'ER' or imp_method == 'MEGA' or imp_method == 'MEGAD') and 'FC-' not in network_arch: # Only for Split-X setups
                self.output_mask = [tf.placeholder(dtype=tf.float32, shape=[self.total_classes]) for i in range(num_tasks)]
...
####################################################################################
    def loss_and_train_ops_for_one_hot_vector(self, x, y_):
        ...
        if self.network_arch == 'FC-S':
            ...
            if self.imp_method == 'PNN':
                ...
                for i in range(self.num_tasks):
                    if i == 0:
                        ...
                        self.unweighted_entropy.append(tf.squeeze(tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_[i], logits=self.task_pruned_logits[i])))) # mult by mean(y_[i]) puts unwaranted loss to 0
                    else:
            ...
            if self.imp_method == 'PNN':
                ...
                for i in range(self.num_tasks):
                    ...
                    self.task_pruned_logits.append(tf.where(tf.tile(tf.equal(self.output_mask[i][None,:], 1.0), [tf.shape(self.task_logits[i])[0], 1]), self.task_logits[i], NEG_INF*tf.ones_like(self.task_logits[i])))
                    self.unweighted_entropy.append(tf.squeeze(tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_[i], logits=self.task_pruned_logits[i]))))
            elif self.imp_method == 'A-GEM' or self.imp_method == 'ER' or self.imp_method == 'MEGA' or self.imp_method == 'MEGAD':
        ...
        if (self.imp_method != 'PNN') and ((self.imp_method != 'A-GEM' and self.imp_method != 'ER' and self.imp_method != 'MEGA' and self.imp_method != 'MEGAD') or 'FC-' in self.network_arch):
        ...
        if 'GEM' not in self.imp_method and 'MEGA' not in self.imp_method and 'MEGAD' not in self.imp_method :
        ...
        # Accuracy measure
        if (self.imp_method == 'PNN') or ((self.imp_method == 'A-GEM' or self.imp_method == 'ER' or self.imp_method == 'MEGA' or self.imp_method == 'MEGAD') and 'FC-' not in self.network_arch):
    ...
    def loss_and_train_ops_for_attr_vector(self, x, y_):
        ...
        if self.imp_method == 'A-GEM':
            ...
            self.unweighted_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_, logits=pruned_zst_logits))
    ...
    def loss_and_gradients(self, imp_method):
        ...
        elif imp_method != 'A-GEM' and imp_method != 'MEGA' and imp_method != 'MEGAD': # For A-GEM we will define the losses and gradients later on
    ...
    def init_vars(self):
        ...
        for v in range(len(self.trainable_vars)):
            ...
            elif self.imp_method == 'A-GEM' or self.imp_method == 'S-GEM' or self.imp_method == 'MEGA'  or self.imp_method == 'MEGAD':
    ...
    def create_fisher_ops(self):
        ...
        self.get_max_fisher_vars = [tf.assign(var, tf.expand_dims(tf.squeeze(tf.reduce_max(scr, keep_dims=True)), axis=0))
                for var, scr in zip(self.max_fisher_vars, self.fisher_diagonal_at_minima)]
        self.get_min_fisher_vars = [tf.assign(var, tf.expand_dims(tf.squeeze(tf.reduce_min(scr, keep_dims=True)), axis=0))
    ...
    def create_stochastic_megad_ops(self):
        ...
        with tf.control_dependencies([flat_task_grads, flat_ref_grads]):
            ...
            g_tilda = tf.cond(tf.less_equal(self.ref_loss, 1e-10), lambda: tf.identity(flat_ref_grads), lambda: flat_task_grads + self.ratio * flat_ref_grads)
