...
## Model options
MODELS = ['VAN', 'PI', 'EWC', 'MAS', 'RWALK', 'M-EWC', 'S-GEM', 'A-GEM', 'FTR_EXT', 'PNN', 'ER', 'MEGA', 'MEGAD'] #List of valid models 
IMP_METHOD = 'EWC'
SYNAP_STGTH = 75000
...
def test_task_sequence(model, sess, test_data, test_tasks, task, classes_per_task=0):
    ...
    if model.imp_method == 'PNN' or model.imp_method == 'A-GEM' or model.imp_method == 'ER' or model.imp_method == 'MEGA' or model.imp_method == 'MEGAD':
    ...
    for tt, labels in enumerate(test_tasks):
        ...
        elif model.imp_method == 'A-GEM' or model.imp_method == 'ER' or model.imp_method == 'MEGA' or model.imp_method == 'MEGAD':
            logit_mask[:] = 0
            logit_mask[tt][labels] = 1.0
            feed_dict = {model.x: task_test_images, 
